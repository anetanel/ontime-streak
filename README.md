...
**Guests (invite links, no login).** Someone she wants to share achievements with opens a link like `https://anetanel.github.io/ontime-streak/?invite=<id>` and is let in invisibly (anonymous Firebase auth under the hood) — they never see the Google sign-in screen. Each invite names exactly one admin's household (`ownerId`, set automatically to whichever admin created it), so a guest only ever sees that one person's achievements — never the other admin's, never check-in history, shifts, or settings, and can't write anything except a comment.
