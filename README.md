...
- **Prizes**: shows how many days until the next prize, the four tiers (every 3 / 7 / 30 / 180 days), and a permanent history of every prize she's won, with the photo, title, tier, and date. She can't add or edit prizes here, but she can now see (with timestamps) and delete comments guests left on each one — a small 🗑️ next to each comment. The prize photo pool itself is managed in the repo, not the app — see `prizes/README.md`.
- **Settings**: celebration sound, and where you back up your data (Export Backup) — do this occasionally and save the file to Files/iCloud or AirDrop it to someone, so history survives a lost phone or a cleared browser. On-time means exactly on time — no grace period.
...
```js
// sw.js
const CACHE_NAME = "ontime-streak-v30"; // increment this
...
- `firestore.rules` (repo root) is the source of truth for security rules. The Firebase CLI is installed standalone (no npm on this machine — `~/tools/firebase-cli/firebase`, on `PATH`) and authenticated; deploy a rules change with `firebase deploy --only firestore:rules` from this directory (`.firebaserc`/`firebase.json` already point it at the `ontime-streak` project). Before the CLI was set up, this required pasting the file by hand into Firebase Console → Firestore Database → Rules, and forgetting that step once caused a real outage — a rule change with no matching deploy fails closed (permission-denied on paths the new rule was supposed to allow). The CLI removes that whole failure mode; there should be no reason to hand-paste rules again.
- `gcloud` is also installed standalone (`~/tools/google-cloud-sdk/bin`, on `PATH`) and authenticated against the same project, for anything on the Google Cloud side (e.g. the API key restriction under APIs & Services → Credentials).
...
**Guests (invite links, no login).** Someone she wants to share achievements with opens a link like `https://anetanel.github.io/ontime-streak/?invite=<id>` and is let in invisibly (anonymous Firebase auth under the hood) — they never see the Google sign-in screen. Each invite names exactly one admin's household (`ownerId`, set automatically to whichever admin created it), so a guest only ever sees that one person's achievements — never the other admin's, never check-in history, shifts, or settings, and can't write anything except a comment. The guest screen shows whose achievements they're looking at (name + photo, captured from the admin's Google account at invite-creation time and stored on the invite doc — `ownerName`/`ownerPhotoURL`) and the guest's own name, plus a **יציאה** (sign out) button — useful if multiple guests share one browser/device, since each needs their own invite link redeemed fresh after the previous guest signs out.

Invites are created in-app now: **Settings → "ניהול הזמנות" (Manage Invites)**. Enter a name (shown next to that guest's comments), tap **"יצירת הזמנה"**, and the link is copied to the clipboard automatically, ready to send. Each admin only ever sees and manages invites to *their own* household — so she creates her own invites for her own achievements, and yours (if you ever make any) are separate. (An invite created before the admin-household split had no `ownerId` field and no longer works.)

There are two distinct actions, and they do different things:
- The **toggle** on an invite deactivates/reactivates the *link* — this only affects whether someone new can redeem it going forward.
- **Deleting** an invite (🗑️) also only stops future redemptions.

Neither of those revokes someone who already redeemed the link — their access (a `guestGrants` doc) is independent of the invite once created. Each invite that has connected guests shows them in a list underneath (name + date joined), with a **🚫** button that actually revokes that specific person — that's the only action that cuts off someone already connected.
